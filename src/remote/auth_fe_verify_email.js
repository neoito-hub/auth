...
export const VerifyEmail = () => {
  ...
  useEffect(() => {
    setEmailLoader(true);
    auth
      .sendOtpForVerification({ user_id: searchParams.get("user_id") })
      .then((response) => {
  ...
  useEffect(() => {
    formik.setFieldValue("user_id", searchParams.get("user_id"));
  ...
  const resendOtp = () => {
    ...
    setCountdownTime(0);
    auth.sendOtpForVerification({ user_id: searchParams.get("user_id") });
  };
